import {
  ...
  export const getActividadesFrecuentesController = async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Error al obtener las actividades frecuentes." });
    }
  };
  ...
  export const getComponentesUsadosController = async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Error al obtener los componentes más usados." });
    }
  };
  ...
  export const getRegistroMantenimientosController = async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Error al obtener el registro de mantenimientos." });
    }
  };
  ...
  export const getMantenimientosPorPeriodoController = async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Error al obtener los mantenimientos por periodo." });
    }
  };
  ...
  export const getEstadisticasMantenimientoController = async (req, res) => {
    try {
      ...
      const estadisticas = await getEstadisticasMantenimiento(fechaInicio, fechaFin);
      res.status(200).json(estadisticas);
    } catch (error) {
      console.error("Error al obtener estadísticas de mantenimiento:", error);
      res.status(500).json({ message: "Error al obtener las estadísticas de mantenimiento." });
    }
  };
  ...
  export const getActivosMasMantenidosController = async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Error al obtener los activos más mantenidos." });
    }
  };
  ...
  export const getDistribucionEstadosController = async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Error al obtener la distribución de estados." });
    }
  };