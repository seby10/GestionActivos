    ...
    debounce(async (inicio, fin, agrup) => {
      ...
      try {
        const [
          ...
          axios.get(
            `http://localhost:3000/api/estadisticas/estadisticasMantenimiento?fechaInicio=${inicio}&fechaFin=${fin}`
          ),
          axios.get(
            `http://localhost:3000/api/estadisticas/activosMasMantenidos?fechaInicio=${inicio}&fechaFin=${fin}`
          ),
          axios.get(
            `http://localhost:3000/api/estadisticas/distribucionEstados?fechaInicio=${inicio}&fechaFin=${fin}`
          ),
          axios.get(
            `http://localhost:3000/api/estadisticas/actividadesFecha?fechaInicio=${inicio}&fechaFin=${fin}`
          ),
          axios.get(
            `http://localhost:3000/api/estadisticas/componentesFecha?fechaInicio=${inicio}&fechaFin=${fin}`
          ),
          axios.get(
            `http://localhost:3000/api/estadisticas/mantenimientosG?fechaInicio=${inicio}&fechaFin=${fin}&agrupacion=${agrup}`
          ),
        ]);

        if (
          !estadisticasRes.data ||
          estadisticasRes.data.total_mantenimientos === 0
        ) {
          setMensajeNulo(
