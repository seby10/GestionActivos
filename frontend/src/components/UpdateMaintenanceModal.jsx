...
import { Dialog, DialogTitle, DialogContent, DialogActions, Button, TextField, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Checkbox, Paper, Typography, Alert, Tooltip, IconButton } from '@mui/material';
...
const UpdateMaintenanceModal = ({ open, onClose, maintenance, onUpdate }) => {
  ...
  const fetchAssetsForAdding = useCallback(async () => {
    if (maintenance) {
      try {
        ...
        const availableAssets = await mantenimientosServices.getAvailableAssets();
        ...
        const combinedAssets = [
          ...
          ...availableAssets.filter(asset => !associatedAssets.some(a => a.ID_ACT === asset.ID_ACT))
            .map(asset => ({ 
              ...asset, 
              isAssociated: false, 
              canRemove: true, 
  ...
  const handleRemoveToggle = (asset) => {
    ...
    if (!validId) {
      ...
      setErrorMessage(`El activo no tiene un ID_ACT o ID_ACT_MANT válido: ${validId}`);
      return;
    }
  
    console.log('Activo seleccionado para eliminar:', asset);
    ...
    if (!asset.toRemove) {
      setAssetsToRemove(prev => [...prev, validId]);
    } else {
      setAssetsToRemove(prev => prev.filter(id => id !== validId));
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    if (assetsToRemove.length > 0) {
      ...
      if (assetsToRemove) {
        const canRemove = await mantenimientosServices.canRemoveAssetFromMaintenance(maintenance.ID_MANT, assetsToRemove);
        if (canRemove) {
          await mantenimientosServices.removeAssetFromMaintenance(maintenance.ID_MANT, assetsToRemove);
          await mantenimientosServices.changeStatusAssetsD(assetsToRemove);
          console.log(`Activo ${assetsToRemove} eliminado del mantenimiento`);
        } else {
          console.log(`No se pudo eliminar el activo ${assetsToRemove} del mantenimiento`);
          setErrorMessage(`No se pudo eliminar el activo ${assetsToRemove} porque tiene actividades o componentes asociados.`);
        }
      }
    }
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      ...
      <DialogContent>
        ...
        {errorMessage && (
          <Alert severity="error" sx={{ mt: 2, mb: 2 }} onClose={() => setErrorMessage('')}>
            {errorMessage}
          </Alert>
        )}
             ...
             <TableRow key={asset.ID_ACT}>
                <TableCell padding="checkbox">
                    <Checkbox
                      ...
                      color="primary"
                      disabled={asset.isAssociated || asset.EST_DET_MANT === 'Finalizado'} 
                    />
                </TableCell>
                <TableCell>
                    <Checkbox
